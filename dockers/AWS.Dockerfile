...
FROM nginx:1.19.7-alpine as builder
...
FROM nginx:1.19.7-alpine as base_image
COPY --from=app ./ .
...
RUN sed -i \
    -e "s/;log_limit = 4096/log_limit = 100000/g" \
...
# RUN composer update --no-scripts --no-plugins
